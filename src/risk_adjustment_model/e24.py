...
class MedicareModelE24(MedicareModel):
    """
    This class represents the E24 ESRD Model for Medicare beneficiaries in beneficiaries
    in dialysis, transplant, and post-graft status. It inherits from the MedicareModel class.
    This is referred to as the 2023 CMS-HCC ESRD risk adjustment models in the CMS
    ...
    def _get_normalization_factor(self, year: int) -> float:
        ...
        """
        norm_factor_dict = {
            2023: 1.127,  # 2023 CMS-HCC ESRD dialysis model for organizations other than PACE: 1.034
            # 2023 CMS-HCC ESRD functioning graft model for organizations other than PACE: 1.048
            2024: 1.146,  # 2023 CMS-HCC ESRD dialysis model (for non-PACE organizations): 1.022
            # 2023 CMS-HCC ESRD functioning graft model (for non-PACE organizations): 1.028
            2025: 1.153,  # 2023 ESRD Dialysis CMS-HCC Model: 1.044
    ...
    def _determine_disease_interactions(
        ...
        """
        # USE BENEFIICARY OBJECT AND POPULATIONS TO RETURN VALID INTERACTIONS

        category_list = [
            category.category for category in categories if category.type == "disease"
        ]
        nonaged = True if beneficiary.age < 65 else False
        ...
        interactions_dict = {
            ...
            "NONAGED_gSubUseDs_gPsych": all(
                [nonaged, g_pyshiatric_v24, g_substance_use_disorder_v24]
            ),
            "NONAGED_HCC6": all([nonaged, "HCC6" in category_list]),
            "NONAGED_HCC34": all([nonaged, "HCC34" in category_list]),
            "NONAGED_HCC46": all([nonaged, "HCC46" in category_list]),
            "NONAGED_HCC110": all([nonaged, "HCC110" in category_list]),
            "NONAGED_HCC176": all([nonaged, "HCC176" in category_list]),
            # --------------
    ...
    def _determine_demographic_interactions(
        ...
        gender = "Female" if beneficiary.gender == "F" else "Male"
        age_group = "Aged" if beneficiary.age >= 65 else "NonAged"

        if beneficiary.fb_dual or beneficiary.pb_dual:
            dual_type = "FB" if beneficiary.fb_dual else "PB"
            interaction = f"{dual_type}Dual_{gender}_{age_group}"
            demo_interactions.append(interaction)

        if beneficiary.orig_disabled:
            demo_interactions.append(f"OriginallyDisabled_{gender}")

        if beneficiary.orec in ["2", "3"] and age_group == "Aged":
            demo_interactions.append(f"Originally_ESRD_{gender}")

        if beneficiary.lti:
            demo_interactions.append(f"LTI_{age_group}")
