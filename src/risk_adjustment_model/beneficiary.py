...
class MedicareBeneficiary(Beneficiary):
    ...
    def __init__(self, gender: str, orec: str, medicaid: bool, population='CNA', age=None, dob=None, diagnosis_codes=[]):
        ...
        self.disabled, self.orig_disabled = self._determine_disabled(self.age, self.orec)
        if self.population == 'NE': # Tim why just NE? what does NE mean?
            self.risk_model_population = self._get_new_enrollee_population(self.risk_model_age, self.orec, self.medicaid)
        else:
